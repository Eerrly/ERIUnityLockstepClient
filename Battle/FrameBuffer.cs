public class FrameBuffer
{
    ...
    public struct Frame
    {
        ...
        public void SetInputByPos(int pos, Input result)
        {
            ...
            System.Console.WriteLine($"FrameBuffer.SetInputByPos pos not found! {pos},{playerCount},{frame}");
        }
        ...
        public bool GetInputByPos(int pos, ref Input result)
        {
            ...
            System.Console.WriteLine($"FrameBuffer.GetInputByPos pos not found! {pos},{playerCount},{frame}");
            return false;
        }
    ...
    public FrameBuffer(int playerCount, int capacity = 1000)
    {
        ...
        System.Console.WriteLine($"Create FrameBuffer playerCount:{playerCount} capacity:{capacity} Input.Size:{size}");
    ...
    public bool TryGetFrame(int frame, ref Frame result, bool reset = true)
    {
        unsafe
        {
            ...
            fixed(byte* dest = &buffer[(frame % capacity) * frameSize])
            {
                ...
                {
                    System.Console.WriteLine($"TryGetFrame must frame by frame lastFrame:{_lastGetFrame.frame} currFrame:{frame}");
                    return false;
                }
                var currentFrame = *(int*)dest;
                if(frame != currentFrame)
        ...
        System.Console.WriteLine($"TryGetFrame _lastGetFrame:{_lastGetFrame.frame}");
        return true;
    }
    ...
    public bool SyncFrame(int frame, ref Frame input, ref int diff)
    {
        unsafe
        {
            ...
            fixed(byte* dest = &buffer[(frame % capacity) * frameSize])
            {
                ...
                if(diff > 1)
                {
                    System.Console.WriteLine($"SyncFrame must frame by frame lastFrame:{_lastSetFrameIndex} currFrame:{frame}");
                    return false;
                }
                if(playerCount > 0)
                {
